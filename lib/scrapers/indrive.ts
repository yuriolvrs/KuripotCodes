...
const EVERYSAVING_URL = "https://www.everysaving.ph/shop/indrive.com";
const OFFICIAL_URL = "https://indrive.com/en-ph";
...
function parseOfficialPage(html: string): RawPromo[] {
  ...
  return uniqueBy(
    codes.map((code) => ({
      platform: "inDrive" as const,
      title: titleFromText("inDrive", code, text),
      code,
...
export const inDriveScraper: Scraper = {
  ...
  async run({ now }) {
    const [officialResult, everysavingResult] = await Promise.allSettled([
      ...
      scrapeEverySaving(EVERYSAVING_URL, "inDrive")
    ]);
    ...
    if (allRawPromos.length === 0) {
      throw new Error("All inDrive requests failed");
    }
